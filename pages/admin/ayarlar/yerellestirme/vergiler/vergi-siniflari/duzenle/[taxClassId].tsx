...
import {
  ADD_TAX_CLASS,
  UPDATE_TAX_CLASS,
} from "../../../../../../../apollo/gql/mutations/localization/tax_class";
...
export default function AddPage() {
  ...
  const [
    getTaxClass,
    { data: taxClassData, loading: taxClassLoading, error: taxClassError },
  ] = useLazyQuery(GET_TAX_CLASS_ADMIN, {
    fetchPolicy: "no-cache",
  });
  ...
  const [
    updateTaxClassRun,
    {
      loading: updateTaxClassLoading,
      error: updateTaxClassError,
      data: updateTaxClassResponse,
    },
  ] = useMutation(UPDATE_TAX_CLASS);
  ...
  useEffect(() => {
    if (router.query.taxClassId) {
      getTaxClass({
        variables: {
          input: {
            id: router.query.taxClassId,
          },
        },
      });
    }
  }, [router.query.taxClassId]);

  useEffect(() => {
    if (
      taxClassData &&
      taxClassData.taxClassOnAdmin &&
      taxClassData.taxClassOnAdmin.id
    ) {
      const {
        id,
        name,
  ...
  useEffect(() => {
    if (
      updateTaxClassResponse &&
      updateTaxClassResponse.updateTaxClass &&
      updateTaxClassResponse.updateTaxClass.id
    ) {
  ...
  if (updateTaxClassLoading || taxRatesLoading || taxClassLoading) {
    return (
