...
import {
  ADD_GEO_ZONE,
  UPDATE_GEO_ZONE,
} from "../../../../../../apollo/gql/mutations/localization/geo_zone";
import { GeoZone } from "../.";
import { GET_GEO_ZONE_ADMIN } from "../../../../../../apollo/gql/query/localization/geo_zone";

export default function UpdateGeoZonePage() {
  ...
  const [
    getGeoZone,
    { data: geoZoneData, loading: geoZoneLoading, error: geoZoneError },
  ] = useLazyQuery(GET_GEO_ZONE_ADMIN, {
    fetchPolicy: "no-cache",
  });
  ...
  const [
    updateGeoZoneRun,
    {
      loading: updateGeoZoneLoading,
      error: updateGeoZoneError,
      data: updateGeoZoneResponse,
    },
  ] = useMutation(UPDATE_GEO_ZONE);
  ...
  useEffect(() => {
    if (router.query.geoZoneId) {
      getGeoZone({
        variables: { input: { id: router.query.geoZoneId } },
      });
    }
  }, [router.query.geoZoneId]);

  useEffect(() => {
    if (geoZoneData && geoZoneData.geoZoneOnAdmin) {
      const {
        id,
        name,
        description,
  ...
  useEffect(() => {
    if (
      updateGeoZoneResponse &&
      updateGeoZoneResponse.updateGeoZone &&
      updateGeoZoneResponse.updateGeoZone.id
    ) {
  ...
  if (updateGeoZoneLoading || countriesLoading || zonesLoading) {
    return (
  ...
  return (
    <SEO
      seo={{
        meta_title: "Bölge Düzenle - Oyuncu Giyim",
        meta_description: "",
        meta_keyword: "",
      }}
    >
